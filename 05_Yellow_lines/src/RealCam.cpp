...
Eigen::Vector3d RealCam::deproject(const cv::Point p) const
{
    rs::intrinsics color_intrin = _dev->get_stream_intrinsics(rs::stream::color);
    rs::float2 depth_pixel = {(float)p.x, (float)p.y};
    rs::float3 depth_point = color_intrin.deproject(depth_pixel, 1);
    Eigen::Vector3d v3;
    v3 << depth_point.x, depth_point.y, depth_point.z;
    return v3;
}